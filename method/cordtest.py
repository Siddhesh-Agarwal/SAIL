...
def find_similar_entity(entities, result_num=4, reverse=True):
    ...
    for k in range(l):
        ...
        for i in range(len(maxlist)):
            k=maxidx[i]-l
            if len(train_text[k].split(' ')) < 3: # Filter out entities with less than 3 words
                continue
...
def predict(idx,num=4,lreverse=True,treverse=True): 
    ...
    with open(os.path.join('../../CORD/test/gpt3_test_cut', file), 'r', encoding='utf-8') as ft:
        data = ft.read().split('\n')
    ...
    for hf in similarimages:
        ...
        with open(os.path.join('../../CORD/train/gpt3_train_cut_gt', sifile),'r',encoding='utf-8') as ft:
            h=ft.read()
            ld+='Document:'+h+'\n\n'
    ...
    for hf in dtexample:
        with open(os.path.join('../../CORD/train/gpt3_train_cut', hf), 'r', encoding='utf-8') as ft:
            h = ft.read()
            hd += 'Q:' + h + ', What are the labels for these texts?\n'
        with open(os.path.join('../../CORD/train/gpt3_train_cut_gt', hf), 'r', encoding='utf-8') as ft:
            h = ft.read()
            hd += 'A:' + h + '\n\n'
