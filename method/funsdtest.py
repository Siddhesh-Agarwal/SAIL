...
def find_similar_text(k,result_num=4, reverse=True):
    ...
    # Order in the prompt
    choosefile=sorted(choosefile, key=lambda x: x[1], reverse=reverse) #True:descending
...
def predict(idx,testpn,num=4): 
    pidx=idx*3+testpn
    test_file_path = "../../FUNSD/testing_data/experience/layoutimage3/{}_{}.jpg".format(idx,testpn) 
    ...
    with open(os.path.join('../../FUNSD/testing_data/gpt3_test_xycut_cut', file), 'r', encoding='utf-8') as ft:
        data = ft.read().split('\n')
    ...
    for hf in similarimages:
        ...
        with open(os.path.join('../../FUNSD/training_data/gpt3_train_xycut_cut_gt', sifile),'r',encoding='utf-8') as ft:
            h=ft.read().split('\n')
            if(pn>=len(h)):
                continue
            h=h[pn]
    ...
    for hf in dtexample:
        ...
        pn = int(hf)%3
        with open(os.path.join('../../FUNSD/training_data/gpt3_train_xycut_cut', tf), 'r', encoding='utf-8') as ft:
            h = ft.read().split('\n')
            if(pn>=len(h)):
                continue
            h=h[pn]
        ...
        with open(os.path.join('../../FUNSD/training_data/gpt3_train_xycut_cut_gt', tf), 'r', encoding='utf-8') as ft:
            h = ft.read().split('\n')[pn]
            hd += 'A:' + h + '\n\n'
...
for idx in range(50): 
    ...
    for testpn in range(3):
        ...
        print(res_text)
        res+=res_text+'\n'
    with open('../../FUNSD/testing_data/result/{}.txt'.format(idx),'w',encoding='utf-8') as ft:
        ft.write(res)
