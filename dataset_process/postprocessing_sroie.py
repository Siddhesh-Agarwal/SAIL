...
root_post = "../../SROIE2019/test/result_post"
root_predres = "../../SROIE2019/test/result_post"
...
# Convert to entity-level label information
for test_data in test_datas:
    ...
    with open(root_test_box_data, "r", encoding="gbk") as fb:
        res_gt_result = os.path.join(root_predres, test_data)
        with open(res_gt_result, "w", encoding="utf-8") as fw:
            ...
            for text, boxes, label in zip(sorted_text, sorted_boxes, sorted_label):
                ...
                # res_words += (
                #     "{" + 'text:"{}",Box:[{}],entity:{}'.format(text, box, label) + "}"
                # )
                res_words += json.dumps({"text": text, "Box": box, "entity": label})
